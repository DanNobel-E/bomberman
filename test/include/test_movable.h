...
CLOVE_TEST(MovableFixPositionX)
{
    level_t level;
    int32_t map[] = {0, 0, 0, 0};
    bmb_level_init(&level, 4, 4, 8, map);
    movable_t movable = { 1, 0, 4, 4};
    bmb_move_on_level(&level, &movable, -10, 0);
...
CLOVE_TEST(MovableFixPositionY)
{
    ...
    bmb_level_init(&level, 4, 4, 8, map);
    movable_t movable = { 1, 0, 4, 4};
    bmb_move_on_level(&level, &movable, 0, -30);
...
CLOVE_TEST(MovableFixPositionWidth)
{
    ...
    bmb_level_init(&level, 4, 4, 8, map);
    movable_t movable = { 1, 0, 4, 4};
    bmb_move_on_level(&level, &movable, 50, 0);
...
CLOVE_TEST(MovableFixPositionHeight)
{
    ...
    bmb_level_init(&level, 4, 4, 8, map);
    movable_t movable = { 1, 0, 4, 4};
    bmb_move_on_level(&level, &movable, 0, 50);
...
CLOVE_TEST(MovableWallHit)
{
    ...
    bmb_level_init(&level, 2, 2, 64, map);
    movable_t movable = { 1, 1, 32, 32};
    bmb_move_on_level(&level, &movable, 80, 0);
